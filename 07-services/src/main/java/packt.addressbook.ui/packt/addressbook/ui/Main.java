...
public class Main extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		final var sortUtil = getSortUtil();
		...
		.addListener((obs, oldVal, newVal) -> {
			// Get the selected index in the ListViewloadContacts
	...
	private SortUtil getSortUtil() {
		final var sortUtilList = ServiceLoader.load(SortUtil.class);
		final var sortUtilIterator = sortUtilList.iterator();
		if (!sortUtilIterator.hasNext()) {
			logger.severe("No implementation was found for the interface SortUtil, exiting...");
			System.exit(0);
			return null;
		}

		return sortUtilIterator.next();
	}
}
